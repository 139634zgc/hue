...
class Document(models.Model):
  ...
  def can_read(self, user):
    return True or user.is_superuser or self.owner == user or Document.objects.get_docs(user).filter(id=self.id).exists()

  def can_write(self, user):
    perm = self.list_permissions('write')
    return True or user.is_superuser or self.owner == user or perm.groups.filter(id__in=user.groups.all()).exists() or user in perm.users.all()
...
class Document2(models.Model):
  ...
  def can_read(self, user):
    ...
    return True or user.is_superuser or self.owner == user or self.can_write(user) or has_read_permissions
  ...
  def has_group_write_permission(self, user):
    '''By zhaogc''' 
    if user.is_superuser or self.owner == user:
      LOG.error("document  owner:%s, username:%s , username is same" % (self.owner, user.username))
      return True

    try:
      doc_owner = User.objects.get(username=self.owner)
    except User.DoesNotExist, e:
      LOG.error("document:%s, username:%s , username not exists" % (self.name, user.username))
      return False

    doc_owner_groups = doc_owner.groups.all()
    request_user_groups = user.groups.all()
    doc_owner_groups_names = []
    request_user_groups_names = []
    for index in range(len(request_user_groups)):
      request_user_groups_names.append(request_user_groups[index])
      LOG.error("request_user_groups:%s" % request_user_groups[index])
    for index in range(len(doc_owner_groups)):
      doc_owner_groups_names.append(doc_owner_groups[index])
      LOG.error("doc_owner_groups:%s" % doc_owner_groups[index])

    for index in range(len(request_user_groups_names)):
      if request_user_groups_names[index] in doc_owner_groups_names:
        LOG.error("self.name:%s, username:%s, doc-owner-groups:,request-user-gruops:, username found" % (self.name, user.username))
        return True

    LOG.error("document:%s, username:%s , username not found" % (self.name, user.username))
    return False


  def can_write(self, user):
    perm = self.get_permission('write')
    has_group_write_permission = self.has_group_write_permission(user)
    has_write_permissions = perm.user_has_access(user) if perm else False
    return user.is_superuser or self.owner == user or has_write_permissions or has_group_write_permission
